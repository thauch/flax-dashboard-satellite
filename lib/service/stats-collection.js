...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'flax-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('flax').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('flax')[service]);
